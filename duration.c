...
void scan_dir(const char *dir_path, int recursive, const char **extensions, int ext_count,
              FileList *list) {
...
int main(int argc, char *argv[]) {
    ...
    if (argc > 1) {
        ...
        for (int i = 1; i < argc; i++) {
            ...
            if (stat(argv[i], &st) == 0) {
                if (S_ISREG(st.st_mode)) {
                    ...
                    scan_dir(argv[i], recursive, mode_video ? video_ext : audio_ext,
        ...
        if (!found_path) {
            scan_dir(".", recursive, mode_video ? video_ext : audio_ext, mode_video ? 5 : 6,
                     &files);
        }
    }
